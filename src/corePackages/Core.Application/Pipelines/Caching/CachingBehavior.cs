    ...
    where TRequest : IRequest<TResponse>, ICachableRequest
{
        ...
        IConfiguration configuration)
    {
        ...
        _logger = logger;
        _cacheSettings =
            configuration.GetSection("CacheSettings").Get<CacheSettings>() ?? throw new InvalidOperationException();
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        byte[]? cacheGroupCacheSlidingExpirationCache = await _cache.GetAsync(key: $"{request.CacheGroupKey}SlidingExpiration",
            cancellationToken);
