    ...
    where TEntity : Entity<TEntityId> where TContext : DbContext
{
    ...
    protected void CheckHasEntityHaveOneToOneRelation(TEntity entity)
    {
        ...
        if (oneToOneForeignKey != null)
        {
            ...
            throw new InvalidOperationException($"Entity {entity.GetType().Name} has a one-to-one relationship with {relatedEntity} via the primary key ({primaryKeyNames}). Soft Delete causes problems if you try to create an entry again with the same foreign key.");
        }
    }

    private async Task setEntityAsSoftDeleted(IEntityTimestamps entity,
        bool isAsync = true,
        CancellationToken cancellationToken = default,
        bool isRoot = true)
    {
            ...
            if (navigation.IsCollection)
            {
                if (navValue == null)
                {
                    ...
                    if (isAsync)
                    {
                        ...
                        navValue = GetRelationLoaderQuery(query, navigationPropertyType: navigation.PropertyInfo
                            .GetType())?
                            .ToList();
