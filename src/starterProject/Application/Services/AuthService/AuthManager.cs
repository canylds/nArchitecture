...
public class AuthManager : IAuthService
{
    ...
    public async Task<AccessToken> CreateAccessToken(User user)
    {
        ...
        AccessToken accessToken = _tokenHelper.CreateToken(user,
            operationClaims.Select(oc => (OperationClaim<int>)oc).ToImmutableList());
    ...
    {
        List<RefreshToken> refreshTokens = await _refreshTokenRepository.GetOldRefreshTokensAsync(userId,
    ...
    {
        RefreshToken? refreshToken = await _refreshTokenRepository.GetAsync(predicate: rt => rt.Token == token);
    ...
    {
        RefreshToken<int, int> newCoreRefreshToken = _tokenHelper.CreateRefreshToken(user, ipAddress);
    ...
    {
        RefreshToken? childToken = await _refreshTokenRepository.GetAsync(predicate: rt =>
        rt.Token == refreshToken.ReplacedByToken);
    ...
    {
        RefreshToken<int, int> coreRefreshToken = _tokenHelper.CreateRefreshToken(user, ipAddress);
